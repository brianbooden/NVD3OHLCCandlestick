...
var viz = function (self, data, labels, width, height, id, selections, layout, $element) {
	...
	dataNVD3 = d3.nest()
				...
				.entries(dataNVD3)
				//.map(function(k){
				//	return {"key": k.key, "values": k.values.map(function(v){return [v.date, v.open, v.high, v.low, v.close, v.volume, v.adjusted]})}
				//})
				;
	...
	console.log(dataNVD3);
	console.log(data);
	
	//$element.html('Original: ' + JSON.stringify(data));
	//$element.html(JSON.stringify(dataNVD3) + '<br><br>' + JSON.stringify(data));
	...
	nv.addGraph(function() {
        ...
        chart.xAxis
				...
				// Use this tickformat for the original data
